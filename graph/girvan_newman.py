...
def stop_condition(g):
    ...
    max_c_size = 10   # !!!!!!!!!!!!!! 10 tunable, can be ration of total number of concepts in two documents
    min_c_size = 3
    if graph_size <= min_c_size:
        return True
    possible_path = min(graph_size * (graph_size - 1) / 2,
                        max_c_size * (max_c_size - 1) / 2)
    # NOTICE: if smaller, the community can be smaller.
    threshold = 1.0 * math.log(possible_path) / math.log(2) + 1 #!!!!!!!!!!!!!! 1,0 tunable
    ...
    if (graph_size > min_c_size and max_betweenness > threshold
        or graph_size > max_c_size):
        return False
    else:
        return True
...
def girvan_newman(G, vprop_name, eprop_weight=None):
    """
    ...
    while g.num_edges() > 0 and g_changed:
        ...
        if len(c_set) < max(3, g.graph_properties["numsent"] / 6.0) or max_size > 10: # 10 tunable>>>>..>>>>>>>>>>>>>
            g_changed = False
            for c_label in c_set:
                # process each connected component
                u = GraphView(g, vfilt=(c.a == c_label))
                if not stop_condition(u) and not is_finalized(u):
                    g_changed = True
